...
**PDF/X-4 needs Ghostscript ≥ 10.05.0, and older builds are refused outright.** In 10.04 and earlier `PDFX` is a *boolean* (its own docs: "the pdfwrite device does not currently support PDF/X versions other than 3"), so `-dPDFX=4` dies with `/typecheck in --pdfmark--`. Worse, a plain CMYK conversion on 10.02.1 — the version Ubuntu 24.04 ships, and therefore what `apt-get install ghostscript` gives CI — leaves `/Group << /CS /DeviceRGB >>` in the output: the objects are DeviceCMYK, but the transparency still composites in RGB. Both were reproduced against the real 10.02.1 binary, not inferred. A Ghostscript that can neither carry an output intent nor finish the colour conversion is not Ghostscript for press purposes, so `colorIntent: "cmyk"` hard-fails on it exactly as it does with none installed. Putting that in `warnings[]` would be the RGB fallback wearing a hat. CI builds 10.07.1 from Artifex's source tarball rather than testing against a version the product refuses.
...
2. ~~Ghostscript 10.x actual PDF/X level support (X-4?) and validation via veraPDF~~ — **ANSWERED 2026-07-10 (Phase 4, `scripts/presstest.js`):** Ghostscript **≥ 10.05.0** produces **PDF/X-4** via `-dPDFX=4` (PDF 1.6, transparency preserved, XMP `pdfxid` identification, fonts intact). Before 10.05, `PDFX` is a boolean and X-4 does not exist; `-dPDFX` (X-1a/X-3) clamps to PDF 1.3 and **flattens the page to a bitmap** — all fonts lost, no extractable text, exit code 0. So the level is X-4, X-1a is not offerable, and the tool requires 10.05.0. **veraPDF does not validate PDF/X at all** (PDF/A + PDF/UA + WTPDF only), so the exit test as planned could never pass; conformance is asserted structurally instead. See §5.
